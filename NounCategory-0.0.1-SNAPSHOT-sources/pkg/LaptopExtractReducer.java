...
public class LaptopExtractReducer extends Reducer<Text, Text,Text, Text> {
	...
	public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException{
		...
		if(product.isEmpty()){
			...
			Path path = new Path(context.getConfiguration().get("LAPTOP_RATING"));
			...
			while((line = reader.readLine())!= null){
				...
				if (lower.contains(laptop)){
					lower = lower.replaceAll("laptop", "NA");
				}
				product.add(lower);
			}
			reader.close();
